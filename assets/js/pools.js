	...
	$.getJSON("//zclassic.mindland.it/services/contents/json.php?wrapjson=true&url=https://data.miningpoolstats.stream/data/zclassic.js?t="+new Date().getTime()+"&callback=?", function(data) {
		if (data) {
			...
			for (var i=0; i < pools.length; i++) {
				var pool = data.data[i];
				if (pool.hashrate > 0) {
					poolHtml += '<tr><td class="center"><img width="20px" src="/assets/images/flags/'+pool.country.split('|')[0].toLowerCase()+'.png" alt="'+pool.country.split('|')[0]+'"/></td><td><a href="'+pool.url+'" target="_blank">' + pool.pool_id + '</td><td>' + pool.country.split('|')[1] +'</td><td class="right">'+((pool.hashrate/poolhashrate)*100).toFixed(2)+'%</td><td class="right">' + (pool.hashrate/1000000).toFixed(4) + '</td></tr>';
				}
			}
			poolHtml += '</tbody>';
			$(".pools").show();
